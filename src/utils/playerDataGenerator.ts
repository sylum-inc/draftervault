...
export const generatePlayerHistoricalData = (
  playerName: string,
  position: string,
  ...
  const data: Array<{ year: string; expected: number; actual: number; differential: number }> = [];
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (randomFactor < 0.15) {
      // 15% chance of injury year
      actualBase *= 0.6;
    } else if (randomFactor > 0.85) {
      // 15% chance of breakout year
      actualBase *= 1.3;
    }
...
export const generateMarketTrendData = (players: ValuedPlayer[]) => {
  ...
  return weeks.map((week) => {
    ...
    return {
      week: `Week ${week}`,
      QB: Math.round(
        (players.filter((p) => p.position === 'QB').reduce((sum, p) => sum + p.estimatedValue, 0) /
          players.filter((p) => p.position === 'QB').length) *
          baseMultiplier
      ),
      RB: Math.round(
        (players.filter((p) => p.position === 'RB').reduce((sum, p) => sum + p.estimatedValue, 0) /
          players.filter((p) => p.position === 'RB').length) *
          baseMultiplier *
          1.1
      ),
      WR: Math.round(
        (players.filter((p) => p.position === 'WR').reduce((sum, p) => sum + p.estimatedValue, 0) /
          players.filter((p) => p.position === 'WR').length) *
          baseMultiplier *
          0.95
      ),
      TE: Math.round(
        (players.filter((p) => p.position === 'TE').reduce((sum, p) => sum + p.estimatedValue, 0) /
...
export const generateTeamNeedsRadarData = (team: { id: string }, allPlayers: ValuedPlayer[]) => {
  const positions = ['QB', 'RB', 'WR', 'TE', 'K', 'DST'];

  return positions.map((position) => {
    const positionPlayers = allPlayers.filter(
      (p) => p.position === position && p.draftedBy === team.id
    );
    const averageValue =
    ...
    const maxPossibleValue = Math.max(
      ...allPlayers.filter((p) => p.position === position).map((p) => p.estimatedValue)
    );
